...
$HanBrakeDirectory = "C:\Users\ADMIN\Desktop\Important Stuff\Tools\Handbreak Encoder"
...
Set-Location $HanBrakeDirectory

#######################################################################################
#Get either the single folder location of a list of dirctories to encode.
$Single12 = $null
while ($Single12 -ne "1" -and $Single12 -ne "2")
{
    $Single12 = Read-Host "Do you want to encode a single folder or multiple folders? (1/2)"
}

#If the user just wants 1 directory get it, if they want multiple get it via a text file.
if ($Single12 -eq 1) {
    $DirectoriesToEncode = Read-Host "Please enter the FDQN of the directory you want to encode"
}
elseif ($Single12 -eq 2) 
{
    #Create the file browser, spawn it, then get the content of the text file
    Write-Host "Please select the text file that contains the list of directories to encode." -ForegroundColor Green
    ...
    $DirectoriesToEncode = Get-Content -Path $FileBrowser.FileName
}
